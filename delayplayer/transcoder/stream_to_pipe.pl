...
#http://open.live.bbc.co.uk/mediaselector/5/select/version/2.0/vpid/bbc_radio_one/mediaset/http-icy-aac-lc-a
...
#return "http://www.bbc.co.uk/radio/listen/live/r1.asx";
...
#	if($chunk =~ /audio_simulcast_uk(.*?)\/>/) 
	{
#		print STDOUT "Got simulcast block\r\n";
...
my $is_running = `ps -A | grep -c stream_to_pipe 2>&1`;

if($is_running>1)
{
	print STDERR "Streamer is already running\n";
